...
void findRegion(int *i_begin, int *i_end, int *bit_index, const std::vector<morton_t> &codes, int i_node) {
    ...
    i_node_end = dir == 1 ? int(codes.size()) : -1;
...
void checkTreesEqual(const std::vector<Node> &nodes_recursive, const std::vector<Node> &nodes,
    ...
    // really ? comparing floats as '==' seems wierd ->
    // /home/ixor/hw/GPGPUTasks2023/src/main_lbvh.cpp:1330: Failure
    // Expected equality of these values:
    //   root_recursive.cmsy
    //     Which is: 487.333
    //   root.cmsy
    //     Which is: 487.333
    // but ok with EXPECT_FLOAT_EQ
...
TEST(LBVH, GPU) {
    ...
    // аналог buildBBoxes
    {
        ...
        for (int i = 0; i < tree_size; ++i) {
            ...
            EXPECT_FLOAT_EQ(nodes[i].mass, nodes_cpu[i].mass);
            EXPECT_FLOAT_EQ(nodes[i].cmsx, nodes_cpu[i].cmsx);
            EXPECT_FLOAT_EQ(nodes[i].cmsy, nodes_cpu[i].cmsy);
            // Expected equality of these values:
            //   nodes[i]
            //     Which is: 36-byte object <9E-86 01-00 3E-0D 03-00 5E-FC 00-00 2A-FE 00-00 11-FE 00-00 CA-FF
            // 00-00 00-00 16-44 AB-44 7D-47 AA-F9 7E-47>
            //                                |
            //   nodes_cpu[i]
            //     Which is: 36-byte object <9E-86 01-00 3E-0D 03-00 5E-FC 00-00 2A-FE 00-00 11-FE 00-00 CA-FF
            // 00-00 00-00 16-44 AB-44 7D-47 AB-F9 7E-47>
            //                                |
            // EXPECT_EQ(nodes[i], nodes_cpu[i]);
        }
    }
